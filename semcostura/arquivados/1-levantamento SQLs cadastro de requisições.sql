...
tpDocumento  int
inMovimentaMateriais  bool S/N? 
ccRecebe
inProcessoRecebe bool S/N? 
contaRecebe
dt_movimento
dt_documento
cdAlmoxarifado
...
--GERAIS:
-- almoxarifadoProcessoExisteLocal
-- tipoMaterialMovimentoLer
-- almoxarifadoProcessoLer  cdProcesso 5
-- lerParametroMaterial
-- naturezaExiste


--POR CAIXA:
-- estoqueAlmoxarifadoLer
-- verificaCaixaDisponivel
-- almoxarifadoProcessoLer  cdProcesso 3



-- Apn..almoxarifadoProcessoExisteLocal
--1 IDM_SP_ALMOXARIFADO_PROCESSO_EXISTE_LOCAL
-- retorna  A04_ID_LOCAL, A04_TX_DESCRICAO 

-- Apn..tipoMaterialMovimentoLer
--2 IDM_SP_TIPO_MOVIMENTA_MATERIAL_LER
retorna A09_TP_DOCUMENTO, A09_IN_MOVIMENTA_MATERIAIS

-- Apn..almoxarifadoProcessoLer
--3.1 IDM_SP_ALMOXARIFADO_PROCESSO_LER
--3.2
nrConta				= M06_NR_CONTA
inProcesso		 	= A02_IN_PROCESSO
cdCcusto 			= M07_CD_CCUSTO

--4 Sybase..IDM_SP_PARAMETRO_MATERIAL_LER
-- lerParametroMaterial
dtMovimento = Trim(ds("M02_TX_PARAMETRO")) & "01"
dtDocumento = Format(Now, "YYYYMMDD")


--5 IDM_SP_NATUREZAS_EXISTE 
Query retorno 1


Detalhes sybase
-- lerParametroMaterial
--4 IDM_SP_PARAMETRO_MATERIAL_LER   Sybase  / {0}: DBSybaseAvenidaBrasil
dtMovimento = Trim(ds("M02_TX_PARAMETRO")) & "01"
dtDocumento = Format(Now, "YYYYMMDD")
...
--6  IDM_SP_ESTOQUE_ALMOXARIFADO_LER
cdAlmoxarifado = A02_ID_ALMOX_PROCES

--3.2 IDM_SP_ALMOXARIFADO_PROCESSO_LER
...
-- 7 IDM_SP_VERIFICA_CAIXA_DISPONIVEL
...
--constante movimento tipo REQUISIÇÃO
...
--5 IDM_SP_NATUREZAS_EXISTE   Sybase  / {0}: DBSybaseAvenidaBrasil
select	1
